...
// Unused
char *D_800DD334[6] = { NULL,       NULL,       NULL, // These could be a file boundary.
...
// Unused?
char *D_800DD410[3] = { D_800E713C, D_800E7140, D_800E7144 };
...
void load_level(s32 levelId, s32 numberOfPlayers, s32 entranceId, s32 vehicleId, s32 cutsceneId) {
    ...
    if (func_8006C2F0() == 0) {
        if (D_800DD32C == 0) {
            ...
            if (gCurrentLevelHeader->race_type == RACE_TYPE_HUB_AREA && gCurrentLevelHeader->world == 0 &&
                !(settings->cutsceneFlags & 0x2000) && settings->wizpigAmulet >= 4) {
    ...
    if ((gCurrentLevelHeader->race_type == RACE_TYPE_DEFAULT || (gCurrentLevelHeader->race_type & RACE_TYPE_CHALLENGE)) &&
    ...
    func_800249F0(gCurrentLevelHeader->geometry, gCurrentLevelHeader->skybox, numberOfPlayers, vehicleId, entranceId,
                  gCurrentLevelHeader->collectables, gCurrentLevelHeader->unkBA);
    if (gCurrentLevelHeader->unk3A == 0 && gCurrentLevelHeader->unk3C == 0 && gCurrentLevelHeader->fogR == 0 &&
        ...
        for (i = 0; i < 4; i++) {
            func_80030664(i, gCurrentLevelHeader->unk3A, gCurrentLevelHeader->unk3C, (u8)gCurrentLevelHeader->fogR,
    ...
    if (gCurrentLevelHeader->weather_enable > 0) {
        func_800AB4A8(gCurrentLevelHeader->unk92, gCurrentLevelHeader->weather_enable, gCurrentLevelHeader->unk96 << 8,
                      gCurrentLevelHeader->unk98 << 8, gCurrentLevelHeader->unk9A << 8, gCurrentLevelHeader->unk94 * 0x101,
...
void func_8006CCF0(s32 arg0) {
    ...
    if (!gIsPaused) {
        ...
        if (func_80066510() == 0 || func_8001139C()) {
            if ((buttonPressedInputs & START_BUTTON) && (func_8006C2F0() == 0) && (D_800DD390 == 0) && (D_801234EC == 0) &&
                (D_80123516 == 0) && (D_800DD394 == 0) && (D_800DD398 == 0)) {
                buttonPressedInputs = 0;
                gIsPaused = TRUE;
                func_80093A40();
            }
...
void func_8006DCF8(s32 arg0) {
    ...
    if ((menuLoopResult & 0x80) && (menuLoopResult != -1)) {
        ...
        // Minor issue here.
        gCurrDisplayList = gDisplayLists[D_801234E8];
        gDPFullSync(gCurrDisplayList++) gSPEndDisplayList(gCurrDisplayList++) temp = menuLoopResult & 0x7F;
        D_80121250[1] = temp;
        D_80121250[0] = D_801234F4;
